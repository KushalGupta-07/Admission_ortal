...
export const Footer = () => {
  return (
    <footer className="bg-gradient-to-br from-gray-900 to-gray-950 text-white pt-20 pb-10 relative overflow-hidden">
      ...
      <div className="absolute bottom-20 right-10 w-96 h-96 bg-accent/5 rounded-full blur-3xl pointer-events-none"></div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['New Registration', 'Application Status', 'Download Admit Card', 'Check Results', 'FAQs'].map((item) => (
                <li key={item}>
                  <a href="#" className="group flex items-center text-gray-400 hover:text-primary transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['User Manual', 'Course Details', 'Fee Structure', 'Important Documents', 'Contact Support'].map((item) => (
                <li key={item}>
                  <a href="#" className="group flex items-center text-gray-400 hover:text-accent transition-colors">
                    <ArrowRight className="h-3 w-3 mr-2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                    {item}
                  </a>
                </li>
          ...
          <div>
            ...
            <div className="flex gap-3 mt-8">
              {[Facebook, Twitter, Linkedin, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="h-10 w-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-primary hover:border-primary hover:scale-110 transition-all duration-300">
